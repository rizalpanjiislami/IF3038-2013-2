...
if($filter == "semua")
{
	$data = mysql_query("SELECT * FROM tugas WHERE kategori REGEXP '$cari' UNION SELECT * FROM tugas WHERE namatugas REGEXP '$cari' UNION SELECT a.kodeuser,a.kodetugas,a.namatugas,a.deadline,a.statustugas,a.kategori FROM tugas AS A INNER JOIN komentar AS B USING(kodeuser,kodetugas) WHERE B.komentar REGEXP '$cari'");
	$user = mysql_query("SELECT * FROM user INNER JOIN profil USING(kodeuser) WHERE username REGEXP '$cari' UNION SELECT * FROM user INNER JOIN profil USING(kodeuser) WHERE email REGEXP '$cari' UNION SELECT * FROM user INNER JOIN profil USING(kodeuser) WHERE nama REGEXP '$cari' UNION SELECT * FROM user INNER JOIN profil USING(kodeuser) WHERE tanggallahir REGEXP '$cari'");
}
...
{
	$data = mysql_query("SELECT * FROM tugas WHERE kategori REGEXP '$cari'");
}
else if($filter == "tugas")
{
	$data = mysql_query("SELECT * FROM tugas WHERE namatugas REGEXP '$cari' UNION SELECT a.kodeuser,a.kodetugas,a.namatugas,a.deadline,a.statustugas,a.kategori FROM tugas AS A INNER JOIN komentar AS B USING(kodeuser,kodetugas) WHERE B.komentar REGEXP '$cari'");
}
...
<?php
	...
	?>
    		<tr>
                ...
                <td>
                	<?php 
						$data2= mysql_query("SELECT * FROM tag WHERE kodeuser='$kodeuser' AND kodetugas='$kodetugas'");
